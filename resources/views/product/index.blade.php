  ...
  <div class='col-md-3'>
    <div class="card card-shadow">
      <div class="card-header card-header-transparent cover overlay" style='position:relative'>
        <img class="cover-image" src="{{isset($product->image)?$product->image:asset('photos/placeholder.png')}}">
      </div>
      <div class="user-base card-block">
        <h4 class="card-title">{{$product->name}}</h4>
        <p class="card-text">Rp {{$product->price}}</p>
        <a href="{{url('products')}}/{{$product->id}}" class="btn btn-sm btn-icon btn-success on-editing cancel-row"><i class="icon wb-eye" aria-hidden="true"></i>&nbsp;&nbsp;Detail</a>
      </div>
    </div>
  </div>
  @endforeach
